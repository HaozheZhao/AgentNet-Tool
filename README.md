...
AgentNet Annotator Platform is a software for humans to annotate agent tasks, which can record the full video with all the keyboard and mouse inputs as well as other important data (HTML, A11ytree elements, ...) during task execution. Agent tasks include all our daily-life computer tasks and professional workflows on computer that can be automated by AI agents.

## Quick Start (Ubuntu)

### Prerequisites

- **Ubuntu 22.04+** with a desktop environment (GUI required)
- **Miniconda/Anaconda** - [Install Miniconda](https://docs.conda.io/en/latest/miniconda.html)
- **Node.js 18+** - Install via:
  ```bash
  curl -fsSL https://deb.nodesource.com/setup_18.x | sudo -E bash -
  sudo apt-get install -y nodejs
  ```
- **OBS Studio** - Required for screen recording:
  ```bash
...
2. **Run the setup script:**
   ...
   ./setup.sh
   ```
   This will create a conda environment named `agentnet` and install all dependencies.

3. **Configure environment variables (for cloud upload):**
   ```bash
   cp .env.example .env
   nano .env  # Edit with your OSS credentials
   ```

4. **Configure OBS Studio:**
   Follow the instructions in [OBS_SETUP.md](OBS_SETUP.md).
...
**Note:** This application requires a graphical display (X11/Wayland). If running on a headless server, use X11 forwarding (`ssh -X`) or VNC.

---

## Alternative: Download Pre-built Application
